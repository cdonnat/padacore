...
public abstract class AdaSourceFile {
	 ...
	 * @return true if the file name (excluding extension) is a valid Ada source file name
	 *         (i.e. contains only letters, digits, underscores and dashes and
	 *         starts with a letter or digit), False otherwise.
	 */
	public final boolean isFileNameValid(String fileNameWithExtension) {
		int extensionLength = this.getExtension().length() + 1; // add leading dot
		int fileNameLastCharIndex = fileNameWithExtension.length() - extensionLength;
		
		String fileNameWithoutExtension = fileNameWithExtension.substring(0, fileNameLastCharIndex);
		boolean isValid = fileNameWithoutExtension.matches("\\A\\p{Alnum}{1}[\\p{Alnum}-_]*");
